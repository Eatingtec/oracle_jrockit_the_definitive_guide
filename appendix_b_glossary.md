...
<a name="adaptive_code_generation"></a>
...
<a name="adaptive_memory_management"></a>
...
<a name="ahead_of_time_compilation"></a>
...
<a name="class_garbage_collection"></a>
...
<a name="code_generation_queue"></a>
## 代码生成队列
...
<a name="conservative_garbage_collection"></a>
...
<a name="constant_pool"></a>
## 常量池（constant pool）

常量池是class文件中的一部分，其中存储了方法所使用到的常量字符串的较大的数字。

<a name="continuous_JRA"></a>
## 持续型JRA
...
<a name="deterministic_garbage_collection"></a>
...
<a name="double_checked_locking"></a>
...
<a name="event_field"></a>
## 事件域（event field）

>参见[事件属性][event_attribute]。

<a name="event_settings"></a>
## 事件配置（event settings）
...
<a name="exact_garbage_collection"></a>
...
<a name="generational_garbage_collection"></a>
...
<a name="jrockit_mission_control"></a>
...
<a name="mixed_mode_interpretation"></a>
...
<a name="native_code"></a>
## 本地代码（native code）

在本书中，本地代码、汇编语言和机器代码是同一个意思，均指针对于指定硬件架构的专用语言。

<a name="native_hypervisor"></a>
...
<a name="out_of_the_box_behavior"></a>
...
<a name="parallel_garbage_collection"></a>
...
<a name="reference_compression"></a>
...
<a name="reference_decompression"></a>
...
<a name="sample_based_profiling"></a>
...
<a name="thread_local_allocation"></a>
...
<a name="tracing_garbage_collection"></a>
...
<a name="virtual_machine_image"></a>
...
<a name="weak_reference"></a>
## 弱引用

>参见[软引用][soft_reference]。

<a name="write_barrier"></a>
## 写屏障（write barrier）
